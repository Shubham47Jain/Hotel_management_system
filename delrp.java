...
public class delrp extends javax.swing.JPanel {
...
ep.setVisible(false);
  ...
  rp.setVisible(true);
    ...
    private void cb1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cb1ActionPerformed
        ...
        if(cb.getSelectedIndex()!=0)
        {
              try
              {
               rs=st.executeQuery("select cost from rooms where rno='"+cb1.getSelectedItem().toString()+"';");
                while(rs.next())
                {
  ...
  rp.setVisible(true);
    ...
    private void butActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_butActionPerformed
        ...
        try
        {
        String ftp=cb.getSelectedItem().toString();
        String fnm=cb1.getSelectedItem().toString();
        
        rs=st.executeQuery("select status from rooms where rno='"+fnm+"';");
        String stu="";
        while(rs.next()){stu=rs.getString(1); }
           ...
           {
       st.executeUpdate("delete from rtype where rtp='"+ftp+"';");
        cb.removeItem(ftp);
        }    

       st.executeUpdate("delete from rooms where rtp='"+ftp+"' AND rno='"+fnm+"' AND cost='"+s1+"';");
                t1.setText("");
                cb.setSelectedIndex(0);

                ep.setVisible(true);
                error.setText("Room Number "+fnm+" has been Removed...");
                rp.setVisible(false);
                rp.setVisible(true); 
            }
            else
